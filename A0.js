...
function Speed(n) {
    return lvA0n[n - 1].add(A0n[n - 1]).mul((new D(2)).pow(lvA0n[n - 1] - 2))
}
...
<div class="A0${n}">
    ...
    每秒生产 ${display(lvA0n[n - 1].add(A0n[n - 1]))} * ${display((new D(2)).pow(lvA0n[n - 1].sub(2)), false)} = ${display(Speed(n))} 个${n == 1 ? "能源" : "生产器"}A<sub>0</sub>${n == 1 ? "" : "<sup>" + (n - 1) + "</sup>"}
...
function DisplayA0U0() {
    return `
<button ${A0U[0] ? "style='background: #ff3f3f'" : ""} ${A0U[0] ? "" : "onmousedown='BuyA0U(0)'"}>
    A<sub>0</sub>U0：
    <br>
    解锁购买最大
    <br>
    花费： 3.4e38 个能源A<sub>0</sub>
</button>`
}
