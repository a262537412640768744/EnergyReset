...
function Alspeedup(l) {
    return Al[l - 1].B.div(8).pow((new D(4)).pow(l - 1))
}
...
function BuyMaxAln(l, n) {
    let v = Al[l - 1].U[4] ? Al[l - 1].A : Al[l - 1].A.add((new D(2)).
        pow((new D(2)).pow(n)).
        pow(Al[l - 1].lvn[n - 1]).
        sub(1).
...
function ExitAlC(l, n) {
    ...
    let A = l == 1 ? A0 : Al[l - 2].A
    let goal = [(new D(2)).pow(69632), new D("1e26000")][n]
    if (A.gte(goal)) {
        Al[l - 1].C[n] = true
    }
    Al[l - 1].currentC = -1
...
<div class="A0${n}">
    ...
    ${DisplayBuyMaxAln(l, n)}
</div>`
}

function DisplayBuyMaxAln(l, n) {
    return Al[l - 1].U[2] ? `
<button onmousedown="BuyMaxAln(${l}, ${n})">
    购买最大
</button>` : ""
}
...
function DisplayBuyMaxAlAll(l) {
    return Al[l - 1].U[2] ? `
<button class="A0" onmousedown="BuyMaxAlAll(${l})">
    全部购买最大
</button>` : ""
}
...
${Al[l - 1].U[n] ? "" : "onmousedown='BuyAlU(" + l + ", " + n + ")'"}>
    ...
    <br>
    ${["A<sub>" + l + "</sub>重置保留低层的升级", "解锁自动购买A<sub>" + (l - 1) + "</sub>和A<sub>" + l + "</sub>挑战", "解锁购买最大", "所有生产器×√2", "购买生产器只需达到并解锁A<sub>" + (l + 1) + "</sub>"][n]}
    <br>
...
onmousedown='${Al[l - 1].currentC == n ? "Exit" : "Enter"}AlC(${l}, ${n})'>
    A<sub>${l}</sub>C${n}：
    <br>
    ${["A<sub>" + (l - 1) + "</sub>宇宙坍塌效果^ " + (l + 0.5), "禁用A<sub>0</sub><sup>8</sup>"][n]}
    <br>
    要求： ${["2.09e20961", "1.00e26000"][n]} 能源A<sub>${l - 1}</sub>
    <br>
    奖励：${["A<sub>0</sub>宇宙坍塌效果^ 0.5 ，并且触发条件^ 1.75", "A<sub>0</sub>宇宙永远不会坍塌"][n]}
    <br>
    点击${Al[l - 1].currentC == n ? ((l == 1 ? A0 : Al[l - 2].A).gte([(new D(2)).pow(69632), new D("1e26000")][n]) ? "完成挑战" : "提前退出") : "进入"}
</button>`
}
